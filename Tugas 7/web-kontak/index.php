...
<head>
    ...
    <style>
        ...
        /* Mencegah tombol di kolom Aksi turun ke bawah */
        td:last-child {
            white-space: nowrap;
        }

        /* Memastikan tombol tampil sejajar */
        .btn-edit,
        .btn-hapus {
...
<body>
    <div class="container">
        <div class="form-wrapper">
            ...
            <div id="statusPesan" style="margin-top: 15px; font-size: 0.9rem; text-align: center;"></div>
        </div>
        ...
        <div class="table-wrapper">
            ...
            <table>
                ...
                <tbody id="isiTabel">
                    <tr>
                        <td colspan="5" style="text-align:center; color:#999; font-style:italic;">Memuat data...</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <div class="modal-overlay" id="modalEdit">
        <div class="modal-box">
            ...
            <form id="formEdit">
                ...
                <button type="button" onclick="tutupModal()" style="background-color:#aaa; margin-top: 10px;">Batal</button>
            </form>
        </div>
    </div>
    ...
    <script>
            ...
            fetch('ambil.php')
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        tabel.innerHTML = `
                    <tr>
                        <td colspan="5" style="text-align:center; color:#999; font-style:italic;">Belum ada data.</td>
                    </tr>`;
                        return;
                    }
                    ...
                    data.forEach(baris => {
                        ...
                        tr.innerHTML = `
                    <td><img src="uploads/${baris.foto}" class="foto-kontak"></td>
                    <td>${baris.nama}</td>
                    <td>${baris.email}</td>
                    <td>${baris.pesan}</td>
                    <td>
                        <button class="btn-edit" onclick="bukaModeEdit(${baris.id})">Edit</button>
                        <button class="btn-hapus" onclick="hapusData(${baris.id})">Hapus</button>
                    </td>
                `;
            ...
            fetch('simpan.php', {
                ...
                .then(data => {
                    if (data.status === 'sukses') {
                        statusPesan.innerHTML = "<span style='color:green;'>Data berhasil disimpan!</span>";
                        form.reset();
                        muatData();
                    } else {
                        statusPesan.innerHTML = "<span style='color:red;'>" + data.pesan + "</span>";
                    }
                    setTimeout(() => {
        ...
        // Fungsi untuk menghapus data dari database (dengan konfirmasi)
        function hapusData(id) {
            if (!confirm('Apakah Anda yakin ingin menghapus data ini?')) {
                return; // Membatalkan proses jika pengguna menekan "Cancel"
            }
            ...
            fetch('hapus.php', {
                ...
                .then(data => {
                    if (data.status === 'sukses') {
                        muatData(); // Refresh tabel setelah data berhasil dihapus
                    } else {
                        alert(data.pesan); // Tampilkan pesan error jika gagal
                    }
                });
        }
    </script>
