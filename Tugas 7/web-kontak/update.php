...
if (isset($_FILES['foto']) && $_FILES['foto']['error'] === 0) {
    ...
    $stmt = mysqli_prepare($koneksi, "SELECT foto FROM kontak WHERE id = ?");
    ...
    $stmt = mysqli_prepare($koneksi, "UPDATE kontak SET nama=?, email=?, pesan=?, foto=? WHERE id=?");
    mysqli_stmt_bind_param($stmt, "ssssi", $nama, $email, $pesan, $nama_file, $id);
    ...
    $stmt = mysqli_prepare($koneksi, "UPDATE kontak SET nama=?, email=?, pesan=? WHERE id=?");
